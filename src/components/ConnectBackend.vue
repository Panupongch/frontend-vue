<template>
    <div>
                    ...
                    <v-container>
                        <v-row>
                            ...
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-model="email" label="จดหมายดิจิทัล"></v-text-field>
                            </v-col>
                        </v-row>
                    </v-container>
                </v-card-text>
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card>
                <v-card-title class="text-h5">ตุณต้องการลบข้อมูลนี้ในตารางไหม</v-card-title>
                <v-card-actions>
                    <v-spacer></v-spacer>
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            { text: 'จดหมายดิจิทัล', value: 'email' },
    ...
    methods: {
        ...
        async deleteItemConfirm() {
            try {
                var response = await this.axios.delete('http://localhost:9000/students/' + this.idforDelete)
                this.initialize()
            } catch (error) {
        ...
        async save(action) {
            ...
            if (action === 'เพิ่มข้อมูล') {
                ...
                try {
                    var dataResponse = await this.axios.put('http://localhost:9000/students/' + this.idEmployee, data)
                    console.log('dataEdit', dataResponse)
                    this.close()
                    this.initialize()
                } catch (error) {
                    console.log(error.message)
